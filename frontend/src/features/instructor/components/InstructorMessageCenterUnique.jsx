...
import styles from '../styles/InstructorMessageCenterUnique.module.css';
import { getDownloadUrl, getFriendlyFileName } from '@/utils/fileUtils';

const InstructorMessageCenterUnique = ({ isOpen, onClose, messages = [], onMessageRead }) => {
    ...
    const confirmDeleteMessage = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/instructor/messages/${confirmDeleteId}`, {
                method: 'DELETE',
                headers: {
    ...
    const handleMessageClick = async (msg) => {
        ...
        if (msg.type === 'received' && !msg.is_read) {
            try {
                const token = getAccessToken();
                const response = await fetch(`/api/instructor/messages/${msg.id}/read`, {
                    method: 'PATCH',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
    ...
    return createPortal(
        <div className={styles.instructorMessageCenter}>
            <div
...
InstructorMessageCenterUnique.propTypes = {
...
export default InstructorMessageCenterUnique;
