...
import styles from '../styles/InstructorDataTable.module.css';
import btnStyles from '@/components/common/styles/Button.module.css';

/**
 * Instructor Data Table Component
 * Supports custom columns, actions, and simple pagination styling, scoped to instructor theme
 */
const InstructorDataTable = ({ columns, data, actions, onAction, emptyMessage, loading, pagination }) => {
        ...
        return (
            <div className={styles.dataTableContainer}>
                <div className={styles.loadingContainer}>
                    ...
                    <p className={styles.loadingText}>{t('dataTable.loading')}</p>
                </div>
            </div>
        );
    }
            ...
            <table className={styles.dataTable}>
                <thead>
                    <tr>
                        ...
                        {actions && <th className={styles.colActions}>{t('dataTable.actionsHeader')}</th>}
                    </tr>
                </thead>
                <tbody>
                    {data.length > 0 ? (
            ...
            {pagination && pagination.total > pagination.limit && (
                <div className={styles.paginationContainer}>
                    <div className={styles.paginationInfo}>
                        {t('dataTable.showing')} {data.length} {t('dataTable.of')} {pagination.total} {t('dataTable.entries')}
                    </div>
                    <div className={styles.paginationControls}>
                        ...
                        <span className={styles.pageInfo}>
                            {t('dataTable.page')} {pagination.page} {t('dataTable.of')} {Math.ceil(pagination.total / pagination.limit)}
                        </span>
                        <button
                            className={styles.btnPagination}
                            onClick={() => pagination.onPageChange(pagination.page + 1)}
